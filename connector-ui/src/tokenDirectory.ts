...
function chainFolderFromIndex(indexJson: any, chainId: number): { chainName: string; sha256: string } {
  ...
  for (const [chainName, meta] of Object.entries<any>(idx)) {
    ...
    if (!sha) throw new Error(`token-directory: no erc20 list for chainId=${chainId} (${chainName})`)
...
export async function resolveErc20Symbol(chainId: number, symbol: string): Promise<Token | null> {
  ...
  const pick = matches.find((t: any) => t?.extensions?.verified === true) || matches.find((t: any) => t?.logoURI) || matches[0]
  return {
